...
dpa=[0.9,1,1+logspace(-3,0,4),linspace(3,4,2)];
...
fid=fopen('ISJ/subadd.txt','w');
xfid=fopen('ISJ/params.cfg','w');
for i1=1:length(dpa)
    for i2=1:length(dpb)
        for i3=1:length(uwa)
            for i4=1:length(lambdar)
                for i5=1:length(files)
                    for i6=1:length(ddist)
                        ...
                        fprintf(xfid,'-i %s.js -o %s.txt -n 100000 -p 6 -u 2 -d %d -h 1 \n',uuids,uuids,ddist(i6));
                        name = fullfile('ISJ','subadd',[uuids,'.js']);
