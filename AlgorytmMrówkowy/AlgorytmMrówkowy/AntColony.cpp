...
//OPCJONALNE
//Pobranie Ÿród³a i celu, potrzebne tylko dla celów demonstracyjnych
void AntColony::getInformation() {
	cout << "Miasto startowe: " << endl;
	cin >> startCity;
	cout << "Miasto docelowe" << endl;
	cin >> stopCity;
}
...
void AntColony::antRelease(int iterator) {
	for (int i = 1; i <= iterator; i++) {
		...
		cout << endl << "Iteracja " << i << " skonczona!" << endl << endl;
	}
}
...
int AntColony::valid(int antNumberK, int iteration) {
	for (int i = 0; i < numberOfNodes-1; i++) {
